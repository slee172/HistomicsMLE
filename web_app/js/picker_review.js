...
$(function() {
	...
	$('#reviewDiag').modal('show');
	$('#reviewprogressBar').css("width", '10%');
...
function genReview() {
	...
	var target = 'genreview';
	$('#reviewprogressBar').css("width", '30%');

  $.ajax({
		type: "POST",
    url: '/model/model/genreview',
		dataType: "json",
		data: {
			uid: uid,
			target: target,
    ...
    success: function(data) {
			...
			$('#reviewprogressBar').css("width", '50%');
			// var reviewJson = JSON.parse(data);
			var reviewJson = data;
			if( reviewJson['status'] === "PASS" ) {
				pickerReviewSample(reviewJson);
			}

			$('#reviewprogressBar').css("width", '80%');
			$('#reviewDiag').modal('hide');
    },
    error: function() {
...
// the selected samples are retrieved by calling reviewSamples.php
//
function pickerReviewSample(reviewJson) {

	var samples = JSON.stringify(reviewJson['picker_review']);
  ...
  $.ajax({
    ...
    success: function(data) {
			...
			sampleDataJson = JSON.parse(data);
			...
			// sort by slide name
			sampleDataJson['picker_review'].sort(function(a, b) {
			...
			for( i = 0; i < sampleDataJson['picker_review'].length; i++ ) {
				boundaries[i] = sampleDataJson['picker_review'][i]['boundary'];
			}
...
function create_mouse_event(sample, slide_num, sample_index){
		...
		box.addEventListener("dragend", function( event ) {
			...
			if (p_id == 'pos_tile'){
				sampleDataJson['picker_review'][box_loc]['label'] = 1;
			}
			else if(p_id == 'neg_tile'){
				sampleDataJson['picker_review'][box_loc]['label'] = -1;
			}
			else{
				sampleDataJson['picker_review'][box_loc]['label'] = 0;
			}

			updateItem.id = sampleDataJson['picker_review'][box_loc]['id'];
			updateItem.label = sampleDataJson['picker_review'][box_loc]['label'];
...
function slidesInfo() {
	...
	var totalNumofCells = sampleDataJson['picker_review'].length;
	var totalNumofPositive = 0;
	var totalNumfofNegative = 0;
	var array_slide = [];

	for( sample in sampleDataJson['picker_review'] ) {
		if (sampleDataJson['picker_review'][sample]['label'] === 1){
			totalNumofPositive = totalNumofPositive + 1;
		}
		else if (sampleDataJson['picker_review'][sample]['label'] === -1){
			totalNumfofNegative = totalNumfofNegative + 1;
		}
		array_slide.push(sampleDataJson['picker_review'][sample]['slide']);
	}
...
function displaySlidesamples(){
	...
	for( sample in sampleDataJson['picker_review'] ) {
		if (isFirst){
			slideObject = [];
			slideName = sampleDataJson['picker_review'][sample]['slide'];
			slideObject.push(sampleDataJson['picker_review'][sample]);
			isFirst = false;
		}
		else{
			if( slideName != sampleDataJson['picker_review'][sample]['slide']){
...
function thumbSingleClick(index) {
	...
	var newSlide = sampleDataJson['picker_review'][index]['slide'];
	curX = Math.round(sampleDataJson['picker_review'][index]['centX']);
	curY = Math.round(sampleDataJson['picker_review'][index]['centY']);
...
function thumbDoubleClick(index) {
	...
	//
	if( label === 1 ) {
		sampleDataJson['picker_review'][index]['label'] = -1;
	} else if( label === -1 ) {
		sampleDataJson['picker_review'][index]['label'] = 1;
	} else {
...
function updateLabels() {
	...
	$('#updateDiag').modal('show');
	$('#updateprogressBar').css("width", '10%');
	...
	var target = 'updatepicker';

  $.ajax({
		type: "POST",
    url: '/model/model/updatepicker',
		dataType: "json",
		data: {
			uid: uid,
			target: target,
    ...
    success: function(data) {
			...
			$('#updateprogressBar').css("width", '80%');
			$('#updateDiag').modal('hide');
